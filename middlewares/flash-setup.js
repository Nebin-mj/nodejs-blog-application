const flashSetup = (req, res, next) => {
   ...
   //to know if a user is signed in or not in the handlebars template to show logout buttot
   ...
   res.locals.userid = req.isAuthenticated() ? req.user.id : false;

   res.locals.diceBear = process.env.DICE_BEAR;
   next();
};
